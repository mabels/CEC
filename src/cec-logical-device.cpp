...
CEC_LogicalDevice::CEC_LogicalDevice(RemoteDebug &_debug)
: CEC_Electrical(_debug)
, Debug(_debug)
...
void CEC_LogicalDevice::OnCECFrame(SerialByte* buffer, int count) {
	CECPacket packet(buffer, count, Debug);
	OnCECPacket(packet);
}

/*
bool CEC_LogicalDevice::TransmitFrame(int targetAddress, const unsigned char* buffer, int count) 
{
	if (_primaryState != CEC_IDLE) {
		Debug.printf("TransmitFrame:abort:%d", _primaryState);
		return false;
	}
...
void CEC_LogicalDevice::OnTransmitComplete(bool success)
{
		...
		_logicalAddress == CLA_UNREGISTERED)
	{
      ...
      Debug.printf("Transmit: %d\n", success);
	}
}
*/

void CEC_LogicalDevice::Handle()
{
	...
	if (((_waitTime == (unsigned long)-1 && !TransmitPending()) || 
	    (_waitTime != (unsigned long)-1 && _waitTime > micros())) && 
		 !IsISRTriggered())
		return;
		*/
